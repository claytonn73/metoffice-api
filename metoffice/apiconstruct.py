...
class baseclass:
    ...
    def _coerce_value(self, value, field_type):
        # checks are ordered by cost of processing
        ...
        if get_origin(field_type) is list:
            ...
            return [self._coerce_value(item, item_type) if item is not None else None for item in value]
        if get_origin(field_type) is dict:
            key_type, value_type = get_args(field_type)
            return {self._convert_key(key_type, k): self._coerce_value(v, value_type) for k, v in value.items()}
    ...
    def __post_init__(self):
        for entry in fields(self):
            ...
            # If an entry is optional, we need to get the actual type from the Union
            if self.is_optional(entry_type):
                entry_type = next(t for t in get_args(entry_type) if t is not NoneType)
            # If the type is a primitive type we skip processing
            if entry_type in (str, int, float, bool):
                continue
            # only get the entry value if the entry is not a primitive type
            entry_value = getattr(self, entry.name)
            # If the value is None, empty string, empty list, or empty dict, we skip processing
            if entry_value in (None, "", [], {}):
                continue
            # Otherwise we process the value based on its type
            setattr(self, entry.name, self._coerce_value(entry_value, entry_type))
